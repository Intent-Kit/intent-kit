---
description: Generate a custom checklist for the current intent based on user requirements, ensuring methodological compliance.
scripts:
...
**After checklist generation and initial validation, execute validation/repair loop:**

1. **Validate Content Quality**: Execute `{VALIDATE_SCRIPT} --file CHECKLIST_FILE --content-type checklist`
   - If validation passes (`status: pass`), proceed to run report generation
   - If validation fails (`status: fail`), continue to repair step

2. **Repair Content (if needed)**: Execute `{REPAIR_SCRIPT} --file CHECKLIST_FILE --content-type checklist`
...
**Upon completion of checklist generation, generate and save run report:**

- **Create run_report.json**: After checklist is validated and written
- **Structure**:
...
**Track checklist generation metrics to calculate Intent Kit Reliability Index:**
