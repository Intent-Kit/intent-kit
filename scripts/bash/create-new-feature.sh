...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h) 
            ...
            echo "  --number N          Intent branch number manually (overrides auto-detection)"
...
check_existing_branches() {
    ...
    # Check intents directory as well
    local intent_dirs=""
    if [ -d "$INTENTS_DIR" ]; then
        intent_dirs=$(find "$INTENTS_DIR" -maxdepth 1 -type d -name "[0-9]*-${short_name}" 2>/dev/null | xargs -n1 basename 2>/dev/null | sed 's/-.*//' | sort -n)
    fi
    
    # Combine all sources and get the highest number
    local max_num=0
    for num in $remote_branches $local_branches $intent_dirs; do
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[intent] Warning: Branch name exceeded GitHub's 244-byte limit"
...
if [ "$HAS_GIT" = true ]; then
    ...
    >&2 echo "[intent] Warning: Git repository not detected; skipped branch creation for $BRANCH_NAME"
fi
