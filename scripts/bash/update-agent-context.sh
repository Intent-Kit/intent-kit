...
CURSOR_FILE="$REPO_ROOT/.cursor/rules/intent-rules.mdc"
...
validate_environment() {
    ...
    if [[ -z "$CURRENT_BRANCH" ]]; then
        ...
        if [[ "$HAS_GIT" == "true" ]]; then
            ...
            log_info "Set INTENT_FEATURE environment variable or create a feature first"
        fi
        exit 1
    fi
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "Make sure you're working on a feature with a corresponding intent directory"
        if [[ "$HAS_GIT" != "true" ]]; then
            log_info "Use: export INTENT_FEATURE=your-feature-name or create a new feature first"
        fi
        exit 1
    fi
