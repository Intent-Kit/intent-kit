...
This guide shows how to iterate on the `intent` CLI locally without publishing a release or committing to `main` first.
...
```bash
git clone https://github.com/Intent-Kit/intent-kit.git
cd intent-kit
...
# Now 'intent' entrypoint is available
intent --help
```
...
```bash
uvx --from . intent init demo-uvx --ai copilot --ignore-agent-tools --script sh
```
...
uvx --from git+https://github.com/Intent-Kit/intent-kit.git@your-feature-branch intent init demo-branch-test --script ps
```
...
```bash
intent check --skip-tls
intent init demo --skip-tls --ai gemini --ignore-agent-tools --script ps
```
